...
public class ClientFS extends FuseStubFS {
     ...
     */
    private RfsMessages.RfsMessageWrapper sendRequest(
            RfsMessages.RfsMessageWrapper request)
    throws IOException {
        ...
        RfsMessages.RfsMessageWrapper respWrapper =
            RfsMessages.RfsMessageWrapper.parseFrom(response);
    ...
    public int readdir(
        ...
        RfsMessages.ReaddirRequest req
            = RfsMessages.ReaddirRequest.newBuilder()
                .setPath(path)
            .build();

        RfsMessages.RfsMessageWrapper reqWrapper
        ...
        RfsMessages.ReaddirResponse rdr = respWrapper.getReaddirResp();
        if (rdr.getStatus() != 0) {
    ...
    public int open(String path, FuseFileInfo fi) {
        ...
        RfsMessages.OpenRequest req
            = RfsMessages.OpenRequest.newBuilder()
                .setPath(path)
            .build();
    ...
    public int getattr(String path, FileStat stat) {
        ...
        RfsMessages.GetattrRequest req
            = RfsMessages.GetattrRequest.newBuilder()
                .setPath(path)
            .build();
        ...
        RfsMessages.GetattrResponse gar = respWrapper.getGetattrResp();
        if (gar.getStatus() != 0) {
            /* Something went wrong on the server side */
    ...
    public int read(String path, Pointer buf,
        ...
        RfsMessages.ReadRequest req
            = RfsMessages.ReadRequest.newBuilder()
                .setPath(path)
                .setSize(size)
                .setOffset(offset)
            .build();
        ...
        RfsMessages.ReadResponse rr = respWrapper.getReadResp();
        if (rr.getStatus() <= 0) {
            /* Something went wrong on the server side */
            return rr.getStatus();
        }
    ...
    public ClientFS(String hostname, int port) {
    ...
    throws IOException {
        ...
        ClientFS fs = new ClientFS(hostname, port);
